...
<article id="post-<?php the_ID(); ?>" 
	<?php post_class( 'post_item post_layout_excerpt post_format_' . esc_attr( $gutentype_post_format ) ); ?>
	...
	gutentype_show_post_featured(
		array(
			...
			'thumb_size' => gutentype_get_thumb_size( strpos( gutentype_get_theme_option( 'body_style' ), 'full' ) !== false ? 'full' : ( $gutentype_expanded ? 'huge' : 'big' ) ),
		)
	);
	...
	if ( empty( $gutentype_template_args['hide_excerpt'] ) ) {
		...
		if ( gutentype_get_theme_option( 'blog_content' ) == 'fullpost' ) {
			...
			?>
				...
				<?php
				// More button
				if ( get_the_title() == '' && empty( $gutentype_template_args['no_links'] ) && ! in_array( $gutentype_post_format, array( 'link', 'aside', 'status', 'quote' ) ) ) {
					?>
