...
if ( ! function_exists( 'gutentype_hovers_theme_setup3' ) ) {
	add_action( 'after_setup_theme', 'gutentype_hovers_theme_setup3', 3 );
	function gutentype_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		gutentype_storage_set_array_after(
			'options', 'border_radius', array(
				'button_hover' => array(
					'title'   => esc_html__( "Button's hover", 'gutentype' ),
					'desc'    => wp_kses_data( __( 'Select hover effect to decorate all theme buttons', 'gutentype' ) ),
					'std'     => 'default',
					'options' => array(
						'default'      => esc_html__( 'Fade', 'gutentype' ),
					),
					'type'    => 'select',
				),
				'image_hover'  => array(
					'title'    => esc_html__( "Image's hover", 'gutentype' ),
					'desc'     => wp_kses_data( __( 'Select hover effect to decorate all theme images', 'gutentype' ) ),
					'std'      => 'simple',
					'override' => array(
						'mode'    => 'page',
						'section' => esc_html__( 'Content', 'gutentype' ),
					),
					'options'  => gutentype_get_list_hovers(),
					'type'     => 'select',
				),
			)
		);
...
if ( ! function_exists( 'gutentype_hovers_theme_setup9' ) ) {
	add_action( 'after_setup_theme', 'gutentype_hovers_theme_setup9', 9 );
	function gutentype_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts', 'gutentype_hovers_frontend_scripts', 1010 );
		add_filter( 'gutentype_filter_localize_script', 'gutentype_hovers_localize_script' );
		add_filter( 'gutentype_filter_merge_scripts', 'gutentype_hovers_merge_scripts' );
		add_filter( 'gutentype_filter_merge_styles', 'gutentype_hovers_merge_styles' );
		add_filter( 'gutentype_filter_merge_styles_responsive', 'gutentype_hovers_merge_styles_responsive' );
		add_filter( 'gutentype_filter_get_css', 'gutentype_hovers_get_css', 10, 2 );
	}
}

// Enqueue hover styles and scripts
if ( ! function_exists( 'gutentype_hovers_frontend_scripts' ) ) {
	function gutentype_hovers_frontend_scripts() {
		if ( gutentype_is_on( gutentype_get_theme_option( 'debug_mode' ) ) ) {
			$gutentype_url = gutentype_get_file_url( 'theme-specific/theme-hovers/theme-hovers.js' );
			if ( '' != $gutentype_url ) {
				wp_enqueue_script( 'gutentype-hovers', $gutentype_url, array( 'jquery' ), null, true );
			}
		}
	}
}
...
if ( ! function_exists( 'gutentype_hovers_merge_styles' ) ) {
	function gutentype_hovers_merge_styles( $list ) {
...
if ( ! function_exists( 'gutentype_hovers_merge_styles_responsive' ) ) {
	function gutentype_hovers_merge_styles_responsive( $list ) {
...
if ( ! function_exists( 'gutentype_hovers_localize_script' ) ) {
	function gutentype_hovers_localize_script( $arr ) {
		$arr['button_hover'] = gutentype_get_theme_option( 'button_hover' );
		return $arr;
	}
}

// Merge hover effects to the single js
if ( ! function_exists( 'gutentype_hovers_merge_scripts' ) ) {
	function gutentype_hovers_merge_scripts( $list ) {
...
if ( ! function_exists( 'gutentype_hovers_add_icons' ) ) {
	function gutentype_hovers_add_icons( $hover, $args = array() ) {
		...
		if ( in_array( $hover, array( 'icons', 'zoom' ) ) ) {
			...
			?>
			<div class="icons">
				<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : esc_attr($no_link); ?>" <?php gutentype_show_layout($target); ?> aria-hidden="true" class="icon-link
									<?php
						...
						apply_filters(
							...
							'<a rel="nofollow" href="' . esc_url( $product->add_to_cart_url() ) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ( 'shop_buttons' == $hover ? ( $product->is_type( 'variable' ) ? esc_html__( 'Select options', 'gutentype' ) : esc_html__( 'Buy now', 'gutentype' ) ) : '' )
										. '</a>',
							$product
						)
					);
                    ...
                    <a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>"
                       aria-hidden="true" class="shop_link button icon-link">
                        ...
                        if ('shop_buttons' == $hover) {
                            if (is_object($args['cat'])) {
                                esc_html_e('View products', 'gutentype');
                            } else {
                                esc_html_e('Details', 'gutentype');
                            }
                        }
                        ?>
                    </a>
			...
			// Hover style 'Icon'
			?>
			<div class="icons"><a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : esc_attr($no_link); ?>" <?php gutentype_show_layout($target); ?> aria-hidden="true" class="icon-search-alt"></a></div>
			<?php

		} elseif ( 'dots' == $hover ) {
			// Hover style 'Dots'
			?>
			<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : esc_attr($no_link); ?>" <?php gutentype_show_layout($target); ?> aria-hidden="true" class="icons"><span></span><span></span><span></span></a>
			<?php

		} elseif ( 'simple' == $hover ) {
			// Hover style 'Dots'
			?>
			<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : esc_attr($no_link); ?>" <?php gutentype_show_layout($target); ?> aria-hidden="true" class="icons"></a>
			<?php
			...
			<div class="post_info">
				<div class="post_info_back">
					...
					if ( ! empty( $post_link ) ) {
						?>
							<a href="<?php echo esc_url( $post_link ); ?>" <?php gutentype_show_layout($target); ?>>
							<?php
					}
						the_title();
					if ( ! empty( $post_link ) ) {
			...
			// Hover style empty
			?>
			<a href="<?php echo esc_url( $post_link ); ?>" <?php gutentype_show_layout($target); ?> aria-hidden="true" class="icons"></a>
			<?php
		}
	}
}
...
// Add styles into CSS
if ( ! function_exists( 'gutentype_hovers_get_css' ) ) {
	function gutentype_hovers_get_css( $css, $args ) {
