<?php
/**
 * The Simple template to display the content
 *
...
<article id="post-<?php the_ID(); ?>" 
	<?php post_class( 'post_item post_layout_simple post_format_' . esc_attr( $gutentype_post_format ) ); ?>
	...
	gutentype_show_post_featured(
		array(
			...
			'thumb_bg'      => true,
			'simple_show'=> true,
			'thumb_size' => gutentype_get_thumb_size( 'full' ),
		)
	);

    ?><div class="wrap_simple"><?php
            ...
            if ( gutentype_get_theme_option( 'blog_content' ) == 'fullpost' ) {
                ...
                ?>
                    ...
                    if ( has_excerpt() ) {
                        ...
                        echo wp_trim_words( $post_excerpt, 20, '...');
                    }
                    ?>
                    </div>
                    <?php
                    ...
                    if ( empty( $gutentype_template_args['no_links'] ) && ! in_array( $gutentype_post_format, array( 'link', 'aside', 'status', 'quote' ) ) ) {
                        ?>
                        <a class="simple-more-link" href="<?php the_permalink(); ?>"><?php esc_html_e( 'Read Article', 'gutentype' ); ?></a>
                        <?php
                    }
            }
            ?>
