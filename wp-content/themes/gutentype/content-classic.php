...
if ( ! empty( $gutentype_template_args['slider'] ) ) {
	...
	echo ( 'classic' == $gutentype_blog_style[0] ? 'column' : 'masonry_item masonry_item' ) . '-1_' . esc_attr( $gutentype_columns );
}
?>
	...
	<?php
		post_class(
			'post_item post_format_' . esc_attr( $gutentype_post_format )
					. ' post_layout_classic post_layout_classic_' . esc_attr( $gutentype_columns )
					. ' post_layout_' . esc_attr( $gutentype_blog_style[0] )
					. ' post_layout_' . esc_attr( $gutentype_blog_style[0] ) . '_' . esc_attr( $gutentype_columns )
		);
	...
	gutentype_show_post_featured(
		array(
			'thumb_size' => gutentype_get_thumb_size(
				'classic' == $gutentype_blog_style[0]
						? ( strpos( gutentype_get_theme_option( 'body_style' ), 'full' ) !== false
								? ( $gutentype_columns > 2 ? 'big' : 'huge' )
								: ( $gutentype_columns > 2
									? ( $gutentype_expanded ? 'med' : 'small' )
									: ( $gutentype_expanded ? 'big' : 'med' )
									)
							)
						: ( strpos( gutentype_get_theme_option( 'body_style' ), 'full' ) !== false
								? ( $gutentype_columns > 2 ? 'masonry-big' : 'full' )
								: ( $gutentype_columns <= 2 && $gutentype_expanded ? 'masonry-big' : 'masonry' )
							)
			),
