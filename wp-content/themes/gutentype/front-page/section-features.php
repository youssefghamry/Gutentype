<div class="front_page_section front_page_section_features<?php
	$gutentype_scheme = gutentype_get_theme_option( 'front_page_features_scheme' );
	if ( ! gutentype_is_inherit( $gutentype_scheme ) ) {
		echo ' scheme_' . esc_attr( $gutentype_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( gutentype_get_theme_option( 'front_page_features_paddings' ) );
?>"
		<?php
		$gutentype_css      = '';
		$gutentype_bg_image = gutentype_get_theme_option( 'front_page_features_bg_image' );
...
<?php
	// Add anchor
	$gutentype_anchor_icon = gutentype_get_theme_option( 'front_page_features_anchor_icon' );
	$gutentype_anchor_text = gutentype_get_theme_option( 'front_page_features_anchor_text' );
...
?>
	...
	<?php
	if ( gutentype_get_theme_option( 'front_page_features_fullheight' ) ) {
	...
	?>
			...
			<?php
			$gutentype_css      = '';
			$gutentype_bg_mask  = gutentype_get_theme_option( 'front_page_features_bg_mask' );
			$gutentype_bg_color_type = gutentype_get_theme_option( 'front_page_features_bg_color_type' );
			if ( 'custom' == $gutentype_bg_color_type ) {
				$gutentype_bg_color = gutentype_get_theme_option( 'front_page_features_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_features_content_wrap content_wrap">
			<?php
			// Caption
			$gutentype_caption = gutentype_get_theme_option( 'front_page_features_caption' );
			if ( ! empty( $gutentype_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_features_caption front_page_block_<?php echo ! empty( $gutentype_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_data( $gutentype_caption ); ?></h2>
				<?php
			}

			// Description (text)
			$gutentype_description = gutentype_get_theme_option( 'front_page_features_description' );
			if ( ! empty( $gutentype_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_features_description front_page_block_<?php echo ! empty( $gutentype_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $gutentype_description ), 'gutentype_kses_content' ); ?></div>
				<?php
			}
			...
			if ( is_active_sidebar( 'front_page_features_widgets' ) ) {
				...
				if ( ! gutentype_exists_trx_addons() ) {
					...
					gutentype_customizer_need_widgets_message( 'front_page_features_caption', 'ThemeREX Addons - Services' );
				}
			}
			?>
			</div>
